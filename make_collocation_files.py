...
def fit_plane_to_7x7_map(map,return_fit=False):
    ...
    # and the residuals (map - plane fit)
    ...
    # the predicted values are the plane fit values plus the center value
    # the center value is the value at the center of the map
    # the center value is returned as a scalar
    #
    ...
    A[:,0] = 1.0
    A[:,1] = np.tile(np.arange(-3,4),(7,1)).T.flatten()
    A[:,2] = np.tile(np.arange(-3,4),(7,1)).flatten()
        ...
        if (ilat > 4) or (ilat < 716):
            ...
            for node in [0,1]:
                ...
                if np.sum(ok) >= frac_num_threshold:
                    ...
                    d_ilat = 4.0*(lat-center_lats[num_found])
                    d_ilon = 4.0*(lon-center_lons[num_found])
                    sat_vap_from_fit = sat_fit[0] + sat_fit[1]*d_ilat + sat_fit[2]*d_ilon
                    era5_vap_from_fit = era5_fit[0] + era5_fit[1]*d_ilat + era5_fit[2]*d_ilon
                    era5_vap_from_fit_sampled = era5_fit_sampled[0] + era5_fit_sampled[1]*d_ilat + era5_fit_sampled[2]*d_ilon
...
ds = xr.Dataset({   'gnss_lats': (['colloc_index'],  gnss_lats),
